...
#+nil (declaim (inline index-posn posn-index posn-column))
...
(defun pretty-out (stream char)
  #+nil (declare (type pretty-stream stream)
	         (type base-char char))
  (cond ((char= char #\newline)
...
(defstruct (logical-block (:type vector) :named)
...
(macrolet ((frob (x) `(cons ',x #'(lambda (object) ,x))))
